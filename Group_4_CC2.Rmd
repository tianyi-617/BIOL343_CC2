...
1. **Tianyi Wang** — 20386987
    + [BIOL343_CC2](https://github.com/tianyi-617/BIOL343_CC2)
1. 
...
```{r import-clean}
data  <- read.csv("datafolder/A1_Data_Excel.csv")
cc1_data <- data[1:46, 1:8]
...
$$ovary(width) \times ovary(height) = ovary(area)$$
...
library(ggplot2)
Graph <- ggplot(aes(ovary_area), data = cc1_data) +
    geom_histogram(binwidth = 0.5) +
    theme_classic()
Graph + facet_wrap(.~population_type)
```
...
![Linked .png file](./imagesfolder/Lycopersicon_pimpinellifolium1.png)

[Source](https://species.wikimedia.org/wiki/Solanum_pimpinellifolium)
